...
class MicrosoftRuntimeConfigService:
    def __init__(
        self,
        *,
    ...
    def resolve(
        ...
        *,
        ...
        oauth = self._safe_microsoft_oauth_settings()
    ...
    def list_accounts(self) -> list[dict[str, object]]:
        try:
            return self._operator_settings().get_microsoft_accounts()
    ...
    def _safe_microsoft_oauth_settings(self) -> dict[str, str | None]:
        try:
            return self._operator_settings().get_microsoft_oauth_settings()
    ...
    def _safe_microsoft_account_settings(self) -> dict[str, str | None]:
        try:
            return self._operator_settings().get_microsoft_account_settings()
        except (ImportError, ModuleNotFoundError, SQLAlchemyError):
            return {
                "account_label": None,
                "account_email": None,
    ...
    def encode_target_value(account_email: str, calendar_id: str) -> str:
        return f"microsoft:{account_email}:{calendar_id}"

    @staticmethod
    def decode_target_value(value: str) -> tuple[str | None, str]:
        raw = str(value or "").strip()
        if not raw.startswith("microsoft:"):
            return (None, raw)
    ...
    def _select_account(
        ...
        *,
        ...
        if account_email:
            ...
            if selected is None:
                raise ValueError(
                    f"Microsoft account '{account_email}' was not found."
                )
            return {
        ...
        if calendar_id:
            ...
            if len(matches) > 1:
                raise ValueError(
                    f"Microsoft calendar target ID '{calendar_id}' is ambiguous across connected accounts."
                )
            if matches:
                match = matches[0]
                return {
        ...
        if calendar_name:
            ...
            if len(matches) > 1:
                raise ValueError(
                    f"Microsoft calendar target name '{calendar_name}' is ambiguous across connected accounts."
                )
            if matches:
                match = matches[0]
                return {
        ...
        account = self._safe_microsoft_account_settings()
        return {
    ...
    def _select_calendar(
        ...
        *,
        ...
        if calendar_id:
            ...
            if selected is None:
                raise ValueError("Microsoft calendar target ID was not found.")
        if selected is None and calendar_name:
            normalized_target = MicrosoftRuntimeConfigService._normalize_calendar_name(
                calendar_name
            )
            matches = [
                item
                for item in calendars
                if MicrosoftRuntimeConfigService._normalize_calendar_name(
            ...
            if len(matches) > 1:
                raise ValueError(
                    f"Microsoft calendar target name '{calendar_name}' is ambiguous."
                )
            if not matches:
                raise ValueError("Microsoft calendar target name was not found.")
            selected = matches[0]
        if selected is None:
            if account_email:
                selected = next(
            ...
            if selected is None:
                selected = next((item for item in calendars if bool(item.get("is_default"))), None)
        if selected is None:
            return {
                "calendar_id": "",
                "calendar_name": "",
