...
def test_booking_setup_page_points_to_apple_reconnect_when_recovery_hints_exist(
    monkeypatch,
) -> None:
    db_path = Path(tempfile.gettempdir()) / f"calsync-ui-test-{uuid4()}.db"
    monkeypatch.setenv("DATABASE_URL", f"sqlite+pysqlite:///{db_path.as_posix()}")
...
def test_connections_page_renders_provider_summary(monkeypatch) -> None:
    ...
    assert (
        "<strong>Alexa:</strong> save the Alexa plan and real skill ID, then save Cloudflare Worker access from the Alexa setup page."
        in response.text
    )
    assert "Finish edge enablement and skill allowlisting before real device traffic is live." not in response.text
...
def test_connections_page_helper_points_to_real_skill_id_when_draft_is_incomplete(
    monkeypatch,
) -> None:
    _configure_test_env(monkeypatch)
    service = OperatorSettingsService(settings=get_settings())
...
def test_alexa_setup_page_helper_points_to_real_skill_id_when_draft_is_incomplete(
    monkeypatch,
) -> None:
    _configure_test_env(monkeypatch)
    service = OperatorSettingsService(settings=get_settings())
...
def test_alexa_simulator_page_points_to_apple_reconnect_when_recovery_hints_exist(
    monkeypatch,
) -> None:
    db_path = Path(tempfile.gettempdir()) / f"calsync-ui-test-{uuid4()}.db"
    monkeypatch.setenv("DATABASE_URL", f"sqlite+pysqlite:///{db_path.as_posix()}")
