...
def test_legacy_backup_recovery_parses_zipped_apple_hints() -> None:
    ...
    assert recovery["recommended_calendar_name"] == "Calendar"
    assert (
        recovery["recommended_calendar_url"]
        == "https://p52-caldav.icloud.com:443/112135872/calendars/6824BCB8-8CEE-4733-9208-4741C62E266C/"
    )
    assert recovery["calendar_count"] == 2
    assert recovery["calendars"][0]["calendar_name"] == "Calendar"
    assert recovery["calendars"][0]["is_writable_hint"] is True
    assert recovery["calendars"][1]["calendar_name"] == "Family"
